...
export class AIGenerationService {
  ...
  async generateComponentContent(request: ContentGenerationRequest): Promise<Array<{
    ...
    // Transform the request to match the Edge Function API
    const componentVariationIds = request.componentVariations.map(v => v.id);
    
    const apiRequest = {
      productName: request.productName,
      productDescription: request.productDescription,
      language: request.language,
      componentVariationIds: componentVariationIds
    };

    console.log('Sending request to generate-component-content:', apiRequest);
  ...
  async generateImage(prompt: string, style: 'realistic' | 'illustration' | 'minimal' = 'realistic'): Promise<string> {
    // Use the new Cloudflare AI image generation service
    const { aiImageService } = await import('./ai-image-generation');
